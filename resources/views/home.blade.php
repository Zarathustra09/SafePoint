            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h5 class="card-title m-0">Recent Login Attempts (All Users)</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover">
...
@push('scripts')
    ...
    <script>
        // Crime Reports Over Time Chart
        const crimeTimeCtx = document.getElementById('crimesTimeChart').getContext('2d');
        const crimesByDay = @json($crimesByDay);

        const timeLabels = crimesByDay.map(item => {
            const date = new Date(item.date);
            return date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        });
        const timeData = crimesByDay.map(item => item.count);
        ...
        // Crime Reports by Severity Chart
        const severityCtx = document.getElementById('crimesSeverityChart').getContext('2d');
        const crimesBySeverity = @json($crimesBySeverity);

        const severityLabels = Object.keys(crimesBySeverity);
        const severityData = Object.values(crimesBySeverity);
        const severityColors = ['#ff6b6b', '#feca57', '#48dbfb', '#1dd1a1'];
